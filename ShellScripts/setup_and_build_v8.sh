...
#
# After running this script, use ./ShellScripts/build.sh for regular project builds.
...
if [ -x "./ShellScripts/build.sh" ]; then
    ./ShellScripts/build.sh $SETUP_FLAGS --build-v8
elif [ -x "$SCRIPT_DIR/build.sh" ]; then
    "$SCRIPT_DIR/build.sh" $SETUP_FLAGS --build-v8
